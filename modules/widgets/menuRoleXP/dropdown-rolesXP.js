 ...
 */
	static async create({ message, content, role, channelID }) {
		...
		for (const buttonObject of role.roles) {
			if(!message.guild.roles.cache.get(buttonObject.role)) throw new Error('Menu Roles: Role not found')
			dropdownsOptions.push(new MessageMenuOption().setEmoji(buttonObject.emoji).setLabel(buttonObject.label).setValue(buttonObject.role).setDescription(`Click para eliminar el role de ${message.guild.roles.cache.get(buttonObject.role).name}!`));
		}

		const dropdown = new MessageMenu().setID('xprl');
